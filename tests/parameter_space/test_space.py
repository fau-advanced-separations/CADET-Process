...
@pytest.mark.parametrize("x,kwargs,expected", [
    ([0.5], {}, True),
    ([-0.1], {}, False),
    ([1.1], {}, False),
    ([1.05], {"tol": 0.1}, True),
], ids=["within", "below", "above", "within_tolerance"])
def test_check_bounds(space_with_column, x, kwargs, expected):
    space_with_column.add_parameter(RangedParameter("a", float, lb=0.0, ub=1.0))
    assert space_with_column.check_bounds(x, **kwargs) is expected
...
def test_evaluate_bounds_includes_dependent_variables(space_with_column, column):
    # evaluate_bounds expects a full vector (independent + dependent);
    # the output shape is 2 * n_parameters, one pair per parameter.
    a = RangedParameter("a", float, lb=0.0, ub=10.0)
...
# ── evaluate_bounds: resolve_dependencies ────────────────────────────────────


def test_evaluate_bounds_resolve_dependencies_feasible(space_with_dependent):
    # Independent x=4.0 → b=2.0; both within [0,10] and [0,5]
    cv = space_with_dependent.evaluate_bounds([4.0], resolve_dependencies=True)
    assert np.all(cv <= 0)


def test_evaluate_bounds_resolve_dependencies_large_independent_feasible(space_with_dependent):
    # x=9.0 → b=4.5, within [0,5]; a=9.0 within [0,10]: no violation
    cv = space_with_dependent.evaluate_bounds([9.0], resolve_dependencies=True)
    assert np.all(cv <= 0)


def test_evaluate_bounds_resolve_dependencies_dependent_violation(space_with_dependent):
    # Build a space where the derived value violates its own bound.
    from dataclasses import dataclass
    ...
    # x=8.0 → b=4.0 > 3.0 → violation
    cv = space.evaluate_bounds([8.0], resolve_dependencies=True)
    assert any(cv > 0)


# ── evaluate_linear_constraints: resolve_dependencies ────────────────────────


def test_evaluate_linear_constraints_resolve_dependencies_feasible(space_with_lincon):
    # [0.4, 0.4]: independent-only; space expands and checks a+b <= 1 → 0.4+0.4-1 = -0.2
    cv = space_with_lincon.evaluate_linear_constraints([0.4, 0.4], resolve_dependencies=True)
    assert np.all(cv <= 0)


def test_evaluate_linear_constraints_resolve_dependencies_violated(space_with_lincon):
    cv = space_with_lincon.evaluate_linear_constraints([0.6, 0.6], resolve_dependencies=True)
    assert cv[0] == pytest.approx(0.2)


# ── evaluate_linear_equality_constraints: resolve_dependencies ────────────────


def test_evaluate_linear_equality_constraints_resolve_dependencies_satisfied(space_with_lineqcon):
    cv = space_with_lineqcon.evaluate_linear_equality_constraints(
        [0.3, 0.7], resolve_dependencies=True
    )
    assert cv == pytest.approx([0.0])


def test_evaluate_linear_equality_constraints_resolve_dependencies_violated(space_with_lineqcon):
    cv = space_with_lineqcon.evaluate_linear_equality_constraints(
        [0.3, 0.5], resolve_dependencies=True
    )
    assert cv == pytest.approx([-0.2])
...
def test_check_bounds_resolve_dependencies_large_independent_feasible(space_with_dependent):
    # x=8.0 → b=4.0, within [0,5]; a=8.0 within [0,10]: still valid
