...
def test_set_values_node_uuids_match_space(two_space, two_models):
    ...
    expected = [two_space.case_uuid(m) for m in two_models]
...
def test_set_values_node_objectless_is_single_sentinel_context():
    ...
    space = ParameterSpace()  # no cases
    node = _make_set_values_node(space)

    contexts = node(x={}, cases=[], cache_nonce=None)

    assert len(contexts) == 1
    assert contexts[0]._uuid == _NO_CASE_UUID
...
def test_evaluate_does_not_print_autogenerated_mapspec(capsys):
    # The set_values node declares its axis-source mapspec explicitly so pipefunc
    # never autogenerates it; autogeneration fires a bare print on every build,
    # which floods stdout on the now-unconditional mapped path.
    space = _make_space(Model(value=1.0), Model(value=2.0))
    pipeline = EvaluationPipeline(space)
    _mapped_scaled(pipeline)
    ...
    assert "Autogenerated" not in capsys.readouterr().out
...
def test_mapped_subset_preserves_request_order():
    ...
    assert pipeline.evaluate({}, cases=[m2, m1])["scaled"] == [20.0, 10.0]
...
def test_mapped_single_object_subset_unwraps():
    ...
    result = pipeline.evaluate({}, cases=[m1])["scaled"]
...
def test_mapped_unknown_object_rejected():
    ...
    with pytest.raises(ValueError, match="Unknown case"):
        pipeline.evaluate({}, cases=[Model(value=9.0)])
