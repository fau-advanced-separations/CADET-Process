...
def run_simulation(process: 'Process', install_path: Optional[str] = None) -> 'SimulationResults':
    """
    ...
    try:
        ...
        if cadet_version < '5.0.0' and branch_name == 'GITDIR-NOTFOUND branch':
            warnings.warn(
                f"Your current CADET-Core release version {cadet_version} may not support all unit operations in this CADET-Process version. "
                f"Please update to the latest CADET-Core version from https://github.com/cadet/CADET-Core for full compatibility.",
                UserWarning
            )
...
@pytest.fixture(scope="class", params=unit_types)
...
@pytest.mark.parametrize("simulation_results", unit_types, indirect=True)
class TestResultsWithLWE:
    ...
    def test_compare_solution_shape(self, simulation_results):
        """
        Test to compare the dimensions of the solution object against the expected solution shape.
        """
        simulation_results = simulation_results
        ...
        if not unit.has_ports:
            ...
            if not isinstance(unit.discretization, NoDiscretization):
                assert simulation_results.solution[unit.name].bulk.solution_shape == (
                    int(process.cycle_time+1), unit.discretization.ncol, process.component_system.n_comp)
        ...
        # FOR UNITS WITH PORTS
            ...
            assert simulation_results.solution[unit.name].bulk.solution_shape == (int(
                process.cycle_time+1), unit.discretization.ncol, unit.n_ports, process.component_system.n_comp)
        ...
        if unit.supports_binding and not isinstance(unit.discretization, NoDiscretization):
            ...
            if 'npar' in unit.discretization.parameters:
                ...
                assert simulation_results.solution[unit.name].solid.solution_shape == (
                    int(process.cycle_time+1), unit.discretization.ncol, unit.discretization.npar, process.component_system.n_comp)
        ...
        if unit.supports_particle_reaction and unit.name != "LumpedRateModelWithoutPores":
            ...
            if 'npar' in unit.discretization.parameters:
                assert simulation_results.solution[unit.name].particle.solution_shape == (
                    int(process.cycle_time+1), unit.discretization.ncol, unit.discretization.npar, process.component_system.n_comp)
            # FOR UNITS WITH PARTICLE MOBILES PHASE AND PARTICLE DISCRETIZATION
            else:
                # ASSERT SOLUTION PARTICLE HAS SHAPE (t, n_col, n_par, n_comp)
                assert simulation_results.solution[unit.name].particle.solution_shape == (
                    int(process.cycle_time+1), unit.discretization.ncol, process.component_system.n_comp)
