...
class ZRMBuilder:
    def __init__(self, component_system, flow_rate, segments_area, configuration, init_c=0, binding_model=None, name=None):
        ...
        if self.flow_rate<0:
            raise ValueError("The flow rate should be positive float")
        self.segments_area = segments_area
        self.init_c=init_c
        self.configuration=configuration.lower()

        if self.configuration not in ['axial', 'radial']:
    ...
    def binding_model(self, model):
        self._binding_model=model
        for membrane_segment in self.zones:
            membrane_segment.binding_model=model
    ...
    def build(self , rate_model , length , porosity, axial_dispersion, volumes_in, volumes_out):
         ...
         elif any(v_in<0 for v_in in volumes_in):
             raise ValueError("The cstr volume in volumes_in can not be negative float.")
    ...
    def _initialize_units(self,rate_model, length, porosity, axial_dispersion, volumes_in, volumes_out):
        ...
        for i in range(self.n):
            zone = rate_model(self.component_system, f"zone_{i+1}")
        ...
        for i in range(self.n):
            cstr_in = Cstr(self.component_system, f"cstr_in_{i+1}")
            cstr_out = Cstr(self.component_system, f"cstr_out_{i+1}")
       ...
       # Conditional handling for configuration
        ...
        for i in range(self.n - 1):
            ...
            self.flow_sheet.set_output_state(
                f'cstr_in_{i+1}',
                {
                    f'cstr_in_{i+2}': next_cstr_fraction,
                    f'zone_{i+1}': zone_fraction
                }
            )
