...
class TrustConstr(SciPyInterface):
    ...
    sparse_jacobian : bool or None, optional
        Determines how to represent Jacobians of the constraints.
        If bool, then Jacobians of all the constraints will be converted to the corresponding format.
        If None (default), then Jacobians won't be converted, but the algorithm can proceed
        only if they all have the same format.
...
class COBYLA(SciPyInterface):
    ...
    Supports:
        - linear constraints.
        - supports_linear_equality_constraints
        - supports_nonlinear_constraints

    Parameters
    ----------
    rhobeg : float, default 1
        Reasonable initial changes to the variables.
    tol_ : float, default 0.0002
...
class SLSQP(SciPyInterface):
    ...
    _specific_options = ['ftol', 'eps', 'disp', 'maxiter', 'finite_diff_rel_step']
