...
class OptimizationProblem:
    ...
    def lower_bounds(self) -> np.ndarray:
        """Lower bounds of independent parameters."""
        return self._space.lower_bounds

    @property
    def upper_bounds(self) -> np.ndarray:
        """Upper bounds of independent parameters."""
        return self._space.upper_bounds

    def evaluate_bounds(self, x: npt.ArrayLike) -> np.ndarray:
        """Return ``[x - lb, ub - x]``; positive entries mean within bounds."""
        x = np.asarray(x, dtype=float).ravel()
        return np.concatenate([x - self.lower_bounds, self.upper_bounds - x])
    ...
    def evaluate_linear_constraints(self, x: npt.ArrayLike) -> np.ndarray:
        ...
        return self._space.A @ np.asarray(x, dtype=float).ravel()
    ...
    def check_linear_constraints(
        ...
        ----------
        x : array-like
            Full parameter vector (or independent-only when
            *get_dependent_values* is True).
        tol : float
            Tolerance added to each bound.
        get_dependent_values : bool
            When True, resolve the full vector from independent values first.
        """
        x = np.asarray(x, dtype=float).ravel()
        if get_dependent_values:
            x = self.get_dependent_values(x)
        A, b = self._space.A, self._space.b
        if A.shape[0] == 0:
            return True
        return bool(np.all(A @ x <= b + tol))

    def evaluate_linear_equality_constraints(self, x: npt.ArrayLike) -> np.ndarray:
        """Return ``Aeq @ x`` for equality constraints."""
        return self._space.A_eq @ np.asarray(x, dtype=float).ravel()
    ...
    def check_linear_equality_constraints(
        ...
        if get_dependent_values:
            x = self.get_dependent_values(x)
        Aeq, beq = self._space.A_eq, self._space.b_eq
        if Aeq.shape[0] == 0:
            return True
        return bool(np.all(np.abs(Aeq @ x - beq) <= tol))
    ...
    def get_chebyshev_center(
        ...
        problem = hopsy.add_box_constraints(
            problem,
            self._space.lower_bounds,
            self._space.upper_bounds,
            simplify=False,
        )
    ...
    def create_initial_values(
        ...
        burn_in: int = 100_000,
        include_dependent_variables: bool = True,
    ) -> np.ndarray:
        ...
        while len(values) < n_samples:
            ...
            full = self.get_dependent_values(ind)

            if not self.check_individual(full, check_nonlinear_constraints=False):
                continue

            values.append(
                full if include_dependent_variables else self.get_independent_values(full)
            )
    ...
    def check_individual(
        ...
        ----------
        x : array-like
            Parameter vector.  May be independent-only (pass
            ``get_dependent_values=True``) or the full vector.
        untransform : bool
            When True, denormalize *x* from normalized coordinates first.
        get_dependent_values : bool
            When True, expand independent values to the full vector first.
    ...
    def evaluate_objectives(
        ...
        ----------
        X : array-like
            1-D (single individual) or 2-D (population) parameter vector in
            physical space.
        untransform : bool
            When True, denormalize *X* from normalized coordinates first.
        get_dependent_values : bool
            When True, resolve dependent variables from independent values.
        ...
        if get_dependent_values:
            X_2d = np.array([self.get_dependent_values(x) for x in X_2d])
    ...
    def check_nonlinear_constraints(
        ...
        """
        x = np.asarray(x, dtype=float).ravel()
        x_full = self.get_dependent_values(x)
        cv = np.atleast_1d(
            np.array(self.evaluate_nonlinear_constraints_violation(x_full), dtype=float)
        )
    ...
    def objective_jacobian(
        ...
        x : array-like
            Parameter values.
            Pass values in physical space when ``untransform=False`` (the
            default), or in transformed (normalized) space when
            ``untransform=True``.
        ...
        get_dependent_values : bool, default=False
            When True, fill in dependent variable values before evaluation.
    ...
    def nonlinear_constraint_jacobian(
        ...
        ----------
        x : array-like
            Parameter values in physical space (``untransform=False``) or
            transformed space (``untransform=True``).
        untransform : bool, default=False
            When True, each finite-difference probe is untransformed before
            evaluation.
        get_dependent_values : bool, default=False
    ...
    def evaluate_meta_scores(
        ...
        ----------
        X : array-like
            1-D (single individual) or 2-D (population) parameter vector.
        untransform : bool
            When True, denormalize *X* from normalized coordinates first.
        get_dependent_values : bool
            When True, resolve dependent variables from independent values.
