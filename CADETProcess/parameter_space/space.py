...
``ParameterSpace`` owns the list of evaluation objects and the list of parameters.
Mappers are wired at ``add_parameter`` time and stored internally; callers interact
...
Normalization
-------------
``normalize`` and ``denormalize`` are thin utilities that apply each
``RangedParameter``'s normalizer element-wise.  They always map between physical
(space-level) bounds and [0, 1].  Constraints are expressed in physical units and are
not automatically transformed.  A linear constraint involving a variable with a
non-linear normalizer becomes non-linear in the normalized coordinate; such a
combination raises when the transformed constraint matrices are assembled by
``TransformedSpace`` (see ``CADETProcess.parameter_space.transformed_space``).
"""
...
class ParameterSpace:
    ...
    def lower_bounds(self) -> np.ndarray:
        """Lower bounds for independent variables; ``-inf`` when unbounded."""
        return np.array(
    ...
    def upper_bounds(self) -> np.ndarray:
        """Upper bounds for independent variables; ``+inf`` when unbounded."""
        return np.array(
    ...
    def sample(
        ...
        problem = hopsy.add_box_constraints(
            problem, self.lower_bounds, self.upper_bounds, simplify=False
        )
